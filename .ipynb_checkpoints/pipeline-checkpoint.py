...
import pickle
from mhcflurry import class1_presentation_predictor
...
def encodingone(data, output):

    label = data["neo_vs_anti"]
    one_hot = data["antigen"].apply(lambda x:pd.Series(list(x)))
    one_hot = pd.get_dummies(one_hot)

    one_hot = pd.merge(one_hot, label, left_index= True, right_index = True)
    one_hot.to_csv(output+'/onehot.csv', index=False)

    return f"saved:{output}"+"/onehot.csv"

def neoantigen_classification(one_hot, output_dir):
    
    neo_data = pd.read_csv(one_hot, index_col=[0])
    train, test = train_test_split(neo_data, test_size=0.2, random_state=25)

    print(f"No. of training samples: {train.shape[0]}")
    print(f"No. of testing samples: {test.shape[0]}")

    x_train = np.array(train.iloc[:, 0:236])
    y_train = np.array(train.iloc[:, -1:])

    x_test = np.array(test.iloc[:, 0:236])
    y_test = np.array(test.iloc[:, -1:])

    opt_accuracy = 0
    for est in range(10, 11):
        for depth in range(5, 6):
            classifier = RandomForestClassifier(n_estimators=est, max_depth=depth, n_jobs=4)
            ...
            tp = 0
            fp = 0
            for y0, y1 in zip(y_test, y_pred):
                if y0 == 1 and y1 == 1:
                    tp += 1
                elif y0 == 0 and y1 == 0:
                    tn += 1
            ...
            if accuracy > opt_accuracy:
                ...
                optimal_n_estimators = est
                optimal_max_depth = depth

            print('Optimal accuracy     = ' + str(opt_accuracy))
            print('Optimal n_estimators = ' + str(optimal_n_estimators))
            print('Optimal max_depth    = ' + str(optimal_max_depth))

    filename = output_dir + '/classification_1.sav'
    
    with open(filename, 'wb') as f:
        pickle.dump(classifier, f)
    print('model saved')

def predict_patient(model, patient):

    x = pd.read_csv(patient)
...
def calc_bind_affinity(data, hla):

    """Calculate binding affinity to given peptides."""

    a = pd.read_csv(data, index_col=None)
    
    peptides = list(a["antigen"])
    alleles = [hla]

    predictor = class1_presentation_predictor.Class1PresentationPredictor.load()
    b = predictor.predict(
        peptides=peptides,
